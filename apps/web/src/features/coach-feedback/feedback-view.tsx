import { feedbackQueue, players } from "@/features/coach-data/mock-data";
...
export function FeedbackView() {
  return <>
    <PageHeader eyebrow="COACHING WORKSPACE" title="피드백" description="선수의 회고에 짧고 구체적인 다음 행동을 연결합니다." action={<ActionButton secondary icon="filter">피드백 템플릿</ActionButton>} />
    <div className="metric-grid four compact">
      ...
      <MetricCard label="미션 연결률" value="81%" helper="피드백 17/21건" tone="purple" />
    </div>
    <div className="content-grid feedback-grid">
      ...
      <Panel title={`${feedbackQueue[0].player.name} · 훈련 리뷰`} description={feedbackQueue[0].session} action={<button className="more-button"><Icon name="more" /></button>}>
        <div className="feedback-detail">
          ...
          <div className="mission-summary"><span><Icon name="target" />오늘의 미션</span><strong>수비 전환 3초 안에 복귀</strong><Badge tone="orange">어느 정도 수행</Badge></div>
          ...
          <div className="next-mission"><label><input type="checkbox" defaultChecked /> 다음 훈련에도 같은 미션 유지</label><button className="action-button"><Icon name="feedback" size={17} />피드백 보내기</button></div>
        </div>
      </Panel>
    </div>
